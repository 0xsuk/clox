...
typedef enum {
  TYPE_FUNCTION, // we're in the body of a function
  TYPE_SCRIPT,   // we're in the top-level code
} FunctionType;
...
static uint8_t makeConstant(Value value) {
  ...
  if (constant > UINT8_MAX) { // Since OP_CONSTANT stores a single byte in the
                              // next byte instruction. UINT8_MAX is the limit
...
static void initCompiler(Compiler *compiler, FunctionType type) {
  ...
  current = compiler;
  if (type != TYPE_SCRIPT) { // TODO: why
    current->function->name =
        copyString(parser.previous.start, parser.previous.length);
  }

  Local *local =
      &current->locals[current->localCount++]; // claims locals[0] for VM's own
                                               // internal use
  local->depth = 0;
...
static ObjFunction *endCompilier() {
  emitReturn();
...
static int resolveUpvalue(Compiler *compiler, Token *name) {
  if (compiler->enclosing == NULL)
    return -1; // variable must be global
  ...
  if (local != -1) {
    ...
    return addUpvalue(
        ...
        true); // isLocal=true means variable is right ouside the function
  }
...
static void declareVariable() {
  if (current->scopeDepth == 0) // function is only for local variable
    return;
...
static void function(FunctionType type) {
  ...
  ObjFunction *function = endCompilier();
...
static void funDeclaration() {
  ...
  markInitialized(); // prevent error by marking as initialized, because
                     // function can be used inside its definition
...
static void expressionStatement() {
  expression();
  consume(TOKEN_SEMICOLON, "Expect ';' after exprssion.");
  emitByte(OP_POP);
}
...
static void forStatement() {
  ...
  if (match(TOKEN_SEMICOLON)) {
    ...
    expressionStatement(); // also consumes ';'
  }
  ...
  // incrementer: we jump over this, go to statement, and then jump to
  // condition, then jump back here (loopStart - incrementStart)
  ...
  statement();
  emitLoop(loopStart); // jump to increment instead of condition, IF incrementer
                       // is present
...
static void or_(bool canAssign) {
  // why not just OP_JUMP ? Why jump when lefthand is false? -> OP_JUMP jumps
  // always. To do similar thing, you need OP_JUMP_IF_TRUE
...
/* paser.current is - */
/* paser.current is 1  */
/* prefixRule for parser.previous "-" is unary */
/* parser.current is *  */
/* prefixRule for parser.previous 1 is number  */
/* pointer to constant 1 is pushed to chunk */
/* precedence (UNARY) is not <= getRule(parser.current.type)->precedence which
...
/* parser.curernt is + */
/* prefixRule for parser.previous 2 is number */
/* pointer to constant 2 is pushed to chunk, number done */
/* FACTOR+1 is not <= getRule(parser.current), so parsePrecedence(FACTOR+1)
